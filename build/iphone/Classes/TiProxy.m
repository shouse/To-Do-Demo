...
//Rare exceptions to indicate a bug in the _to_do code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_to_do = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_to_do]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.to_do.module"
